 ...
 * @description 通用消息对象，基于 Map 实现的可嵌套数据结构，支持 JSON 数据结构
...
public class JSONObject extends LinkedHashMap<String, Object> {
    ...
    /**
     * 将所有层次中凡是 Map 类型同时又不是 MessageObject 的类型，转换成 MessageObject 类型
     *
     * @param element
     * @return 转换后的值
     */
    @SuppressWarnings("unchecked")
    private static Object transfer(final Object element) {
        if (!(element instanceof JSONObject) && element instanceof Map) {
            return toObj((Map<String, Object>) element);
        } else if (!(element instanceof JSONArray) && element instanceof Collection) {
            return toArr((Collection<Object>) element);
        } else {
            return element;
        }
    }
    ...
    /**
     * 获取指定字段的整数值，如果字段不存在，或者无法转换为整数，返回 defaultValue
     *
     * @param name
     * @param defaultValue
     * @return
    ...
    /**
     * 获取对象（非标量类型）字段。返回的数据是一个结构体。当不存在指定对象时，则为指定的名字创建一个空的 MessageObject 对象
     *
     * @param name 字段名 不支持多级名字，支持数组下标
     * @return 返回指定的对象。如果对象不存在，则为指定的名字创建一个空的 MessageObject 对象
     */
    ...
    /**
     * 获取数组字段。将名字对应的对象以数组对象返回。当指定的对象不存在时，创建一个空的数组。
     *
     ...
     *
     * @param name  字段名
     * @param value 字段值（标量：数字、字符串、布尔值；结构体：MessageObject）。
     *              如果是 Map 类型同时非 MessageObject 类型，则自动转换为 MessageObject 类型再存入
     *              （此时，再修改 Map 中的数据，将不会体现到本对象中）
     * @return 返回本对象
     */
     ...
     * 将本对象转换为 Java Bean
     *
     * @param beanClass Java Bean 的类对象
     * @return 返回转换后的 Java Bean
     */
    ...
    /**
     * 重载基类的方法。如果 value 是 Map 类型，但不是 MessageObject 类型，则创建一个包含内容等同于原 Map 的 MessageObject 作为 value
     * （伺候在更改 Map 的内容，将不会反映到 MessageObject 中）
     * 重载此方法的目的是为了使 JSON 能够正确地解析为 MessageObject 对象。不建议直接调用此方法，请使用 set(name, value) 方法设置字段值。
     */
    @Override
    ...
    public static class JSONArray extends ArrayList<Object> {
        ...
        public boolean add(Object object) {
            return super.add(transfer(object));
        }
